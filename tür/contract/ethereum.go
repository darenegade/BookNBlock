...
//Zugeordnete Tür hat eine eindeutige ID und ist einem Vermieter zugeordnet
type Door struct {
	LandlordID string
	RenterID   string
	DoorID     string
...
func (e *Ethereum) IsAllowedAt(renterID string, t time.Time) (allowed bool) {
	//Annahme: Es existiert ein Contract mit der Methode 'isAllowed(doorID string, renterID string, time time.Time)'
	//	allowed = e.contract.isAllowedAt(e.door.DoorID, e.door.renterID,t)
	return false
}
func (d *Door) setDoorID(doorID string) {
	d.DoorID = doorID
}
